from flask import (
    Blueprint, render_template, flash, url_for, redirect, request
)
...
def hello_world(end_point):
    ...
    if end_point == 'world':
        ...
        scope = 'hello'
        redirect_url = 'http://localhost:5000/vat/authorisation'
        hmrc_auth_url = 'https://test-www.tax.service.gov.uk/oauth/authorize?'
        auth_params = 'response_type=code&client_id=%s&scope=%s&redirect_uri=%s' % (client_id, scope, redirect_url)
...
def token_exchange():
    ...
    payload = {
        ...
        'redirect_uri': 'http://localhost:5000/vat/authorisation',
    ...
    r = requests.get('https://test-api.service.hmrc.gov.uk/hello/user', json=payload, headers=headers)
